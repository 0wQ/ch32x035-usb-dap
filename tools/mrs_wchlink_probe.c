...
typedef int (*compare_version_fn)(const char *, int *, int *);
...
typedef int (*download_ex_fn)(int, int, int, const char *);
typedef int (*flash_operation_fn)(int, int, int, int, const char *);
typedef int (*flash_operation_ex_fn)(int, int, int, int, const char *);
typedef int (*flash_operation_external_fn)(int, int, int, int, const char *, const char *);
typedef int (*flash_operation_dap_fn)(int, int, int, int, const char *);
typedef int (*clear_code_flash_fn)(int, int, int);
typedef int (*clear_code_flash_dap_fn)(int, int, int);
...
static const char *default_library_path =
    "/Applications/MounRiver Studio 2.app/Contents/Resources/app/resources/"
...
static const char *default_wchlink_config_path =
    "/Applications/MounRiver Studio 2.app/Contents/Resources/app/resources/darwin/"
    "components/WCH/Others/Firmware_Link/default/wchlink.wcfg";
...
static void print_usage(const char *program) {
    fprintf(stderr,
            "usage:\n"
            "  %s preflight [--library PATH] [--serial SERIAL] [--location BUS-PORT]\n"
            "  %s preflight-configured [--library PATH] [--serial SERIAL] [--location BUS-PORT]\n"
            ...
            "  %s download-ex|download-ex-verify --file PATH [--address N] [--library PATH]\n"
            "             [--serial SERIAL] [--location BUS-PORT] [--family N]\n"
            "             [--debug-mode N] [--speed N]\n"
            ...
            "  %s flash-basic|flash-dap|flash-external --file PATH [--flags N] [--address N]\n"
            "  %s flash-linked|flash-open|flash-gui --file PATH [--flags N] [--address N]\n"
            "  %s linked-id [--library PATH] [--serial SERIAL] [--location BUS-PORT]\n"
            "  %s erase-dap [--clear-type N] [--library PATH] [--location BUS-PORT]\n"
            "            [--serial SERIAL] [--family N] [--debug-mode N] [--speed N]\n"
            "  %s set-mem-type-only [--library PATH] [--serial SERIAL]\n"
            "            [--family N] [--debug-mode N] [--speed N]\n"
            "  %s get-device-mode-only [--library PATH] [--serial SERIAL]\n"
            "            [--family N] [--debug-mode N]\n"
            "\n"
            "default serial: %s\n"
...
#define LOAD(name, field)                                                    \
    ...
    LOAD("McuCompiler_CompareVersion", compare_version)
    ...
    LOAD("McuCompiler_DownloadEx", download_ex)
    LOAD("MRSFunc_FlashOperation", flash_operation)
    LOAD("MRSFunc_FlashOperationExB", flash_operation_ex)
    LOAD("MRSFunc_FlashOperationExternal", flash_operation_external)
    LOAD("MRSFunc_FlashOperation_DAP", flash_operation_dap)
    LOAD("MRSFunc_ClearCodeFlash", clear_code_flash)
    LOAD("MRSFunc_ClearCodeFlash_DAP", clear_code_flash_dap)
...
static int open_and_configure(struct mrs_api *api, int family, int debug_mode, int speed,
    ...
    result = api->set_chip_type(family, subtype, skip_chip_info_query);
    printf("set_chip_type(%d,skip_info=%d)=%d subtype=0x%02x\n", family,
           skip_chip_info_query ? 1 : 0, result, *subtype);
    return result;
}

static int run_gui_preflight(struct mrs_api *api) {
    int link_type = 0;
    int link_mode = 0;
    int result = api->open_device();

    printf("gui_open_device=%d\n", result);
    if (result != 0) {
        return result;
    }

    // GUI 以版本文件确认 Link 固件后关闭设备，随后 FlashOperationExB 自行重新打开
    result = api->compare_version(default_wchlink_config_path, &link_type, &link_mode);
    printf("gui_compare_version=%d type=%d mode=%d\n", result, link_type, link_mode);
    api->close_device();
    // CompareVersion 的非负返回值不足以单独判断 GUI 是否弹出升级提示
...
static int run_configured_gui_preflight(struct mrs_api *api, int family, int debug_mode) {
    ...
    return run_gui_preflight(api);
}

static int run_linked_id(struct mrs_api *api) {
    int link_type = 0;
    int link_mode = 0;
    int result = api->open_device();
    printf("open=%d\n", result);
...
int main(int argc, char **argv) {
    ...
    for (int index = 2; index < argc; ++index) {
        ...
        } else if (strcmp(argv[index], "--skip-chip-info") == 0) {
            skip_chip_info_query = true;
    ...
    if (strcmp(command, "flash") == 0 || strcmp(command, "verify") == 0 ||
        strcmp(command, "download") == 0 || strcmp(command, "download-ex") == 0 ||
        strcmp(command, "download-ex-verify") == 0) {
        if (file_path == NULL) {
    ...
    } else if (strcmp(command, "preflight") != 0 &&
               strcmp(command, "preflight-configured") != 0 &&
               strcmp(command, "query-rprotect-only") != 0 &&
               strcmp(command, "get-mem-type-only") != 0 &&
               strcmp(command, "set-mem-type-only") != 0 &&
               strcmp(command, "get-device-mode-only") != 0 &&
    ...
    if (strcmp(command, "preflight") == 0) {
        result = run_gui_preflight(&api);
        unload_api(&api);
        return result == 0 ? 0 : 1;
    }

    if (strcmp(command, "preflight-configured") == 0) {
        result = run_configured_gui_preflight(&api, family, debug_mode);
        unload_api(&api);
        return result == 0 ? 0 : 1;
    }
    ...
    if (strcmp(command, "query-rprotect-only") == 0 ||
        strcmp(command, "get-mem-type-only") == 0 ||
        strcmp(command, "set-mem-type-only") == 0 ||
        strcmp(command, "get-device-mode-only") == 0) {
        ...
        printf("set_target_chip(%d,%d)=%d\n", family, debug_mode, result);
        if (result == 0) {
            if (strcmp(command, "get-device-mode-only") == 0) {
                result = api.open_device();
                printf("open=%d\n", result);
                if (result == 0) {
                    result = api.get_device_mode();
                    printf("get_device_mode=%d\n", result);
                    api.close_device();
                    printf("close=0\n");
                }
            } else if (strcmp(command, "query-rprotect-only") == 0) {
                result = api.query_protection(family, speed);
                printf("query_rprotect=%d\n", result);
            } else if (strcmp(command, "get-mem-type-only") == 0) {
                result = api.get_mem_type(family, speed);
                printf("get_mem_type=%d\n", result);
            } else {
                result = api.set_mem_type(family, speed, 104);
                printf("set_mem_type=%d\n", result);
            }
        }
        unload_api(&api);
        return result == 0 ? 0 : 1;
    ...
    {
        if (strcmp(command, "linked-id") == 0) {
            result = run_linked_id(&api);
        } else if (strcmp(command, "download") == 0 || strcmp(command, "download-ex") == 0 ||
                   strcmp(command, "download-ex-verify") == 0) {
            uint8_t subtype = 0u;
            result = open_and_configure(&api, family, debug_mode, speed, &subtype);
            printf("configure_result=%d subtype=0x%02x\n", result, subtype);
            if (result == 0) {
                if (strcmp(command, "download-ex") == 0 ||
                    strcmp(command, "download-ex-verify") == 0) {
                    int operation = strcmp(command, "download-ex-verify") == 0 ? 1 : 0;
                    result = api.download_ex(operation, family, address, file_path);
                } else {
            ...
            api.close_device();
        } else if (strcmp(command, "flash") == 0 || strcmp(command, "flash-basic") == 0 ||
                   strcmp(command, "flash-dap") == 0 ||
                   strcmp(command, "flash-external") == 0 || strcmp(command, "flash-linked") == 0 ||
                   strcmp(command, "flash-open") == 0 || strcmp(command, "flash-gui") == 0) {
            int operation_family = family;
            bool keep_open = strcmp(command, "flash-open") == 0;
            bool gui_preflight = strcmp(command, "flash-gui") == 0;
            if (keep_open) {
                uint8_t subtype = 0u;
                result = open_and_configure(&api, family, debug_mode, speed, &subtype);
                printf("configure_result=%d subtype=0x%02x\n", result, subtype);
            ...
            if (strcmp(command, "flash-linked") == 0) {
                result = run_linked_id(&api);
                if (result != 0) {
                    unload_api(&api);
                    return 1;
                }
                operation_family = api.get_chip_id();
                printf("using_linked_mcu_id=%d (0x%x)\n", operation_family, operation_family);
            }
            if (!keep_open) {
                result = api.set_target_chip(operation_family, debug_mode);
                printf("set_target_chip(%d,%d)=%d\n", operation_family, debug_mode, result);
            } else if (strcmp(command, "flash-basic") == 0) {
                result = api.flash_operation(operation_family, speed, flags, address, file_path);
            } else {
                result = 0;
            }
            if (result != 0) {
            ...
            printf("flash_operation(family=%d,speed=%d,flags=0x%02x,address=0x%08x,file=%s)\n",
                   operation_family, speed, flags, (unsigned int)address, file_path);
            if (strcmp(command, "flash-dap") == 0) {
                result = api.flash_operation_dap(operation_family, speed, flags, address, file_path);
            } else if (strcmp(command, "flash-external") == 0) {
                result = api.flash_operation_external(operation_family, speed, flags, address,
                                                       file_path, NULL);
            } else {
                result = api.flash_operation_ex(operation_family, speed, flags, address, file_path);
            }
            printf("%s_result=%d\n", command, result);
            if (keep_open) {
                api.close_device();
        ...
        } else if (strcmp(command, "erase") == 0 || strcmp(command, "erase-dap") == 0) {
            result = api.set_target_chip(family, debug_mode);
            printf("set_target_chip(%d,%d)=%d\n", family, debug_mode, result);
            if (result != 0) {
                unload_api(&api);
            ...
            if (strcmp(command, "erase-dap") == 0) {
                result = api.clear_code_flash_dap(family, speed, clear_type);
            } else {
                result = api.clear_code_flash(family, speed, clear_type);
            }
            printf("%s(%d,%d,%d)=%d\n", command, family, speed, clear_type, result);

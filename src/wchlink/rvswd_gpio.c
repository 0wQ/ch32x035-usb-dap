...
#define RVSWD_DMI_BUSY_DELAY_MS 4u
...
bool rvswd_gpio_write_dmi(uint8_t address, uint32_t value) {
    ...
    for (uint8_t retry = 0u; retry < RVSWD_DMI_WRITE_RETRY_COUNT; ++retry) {
        ...
        bsp_delay_ms(RVSWD_DMI_BUSY_DELAY_MS);
    }
...
bool rvswd_gpio_read_memory32(uint32_t address, uint32_t *value) {
    ...
    if (!rvswd_gpio_write_dmi(0x20u, 0x0002a303u) ||
        ...
        !rvswd_gpio_write_dmi(0x17u, 0x00271005u) ||
        !rvswd_gpio_write_dmi(0x17u, 0x00221006u) ||
...
bool rvswd_gpio_write_memory32(uint32_t address, uint32_t value) {
    ...
    // 使用目标调试模块的程序缓冲区执行一次 32 位内存写入
    if (!rvswd_gpio_write_dmi(0x20u, 0x0072a023u) ||
        !rvswd_gpio_write_dmi(0x21u, 0x00100073u) ||
        !rvswd_gpio_write_dmi(0x04u, address) ||
        !rvswd_gpio_write_dmi(0x16u, 0x00000700u) ||
        !rvswd_gpio_write_dmi(0x17u, 0x00231005u) ||
        !rvswd_gpio_write_dmi(0x04u, value) ||
        !rvswd_gpio_write_dmi(0x16u, 0x00000700u) ||
        !rvswd_gpio_write_dmi(0x17u, 0x00271007u) ||
...
bool rvswd_gpio_flash_erase_all(void) {
    ...
    if (!rvswd_gpio_flash_unlock() ||
        !rvswd_gpio_read_memory32(0x40022010u, &control) ||
        !rvswd_gpio_write_memory32(0x40022010u, control | (1u << 2u)) ||
        !rvswd_gpio_write_memory32(0x40022010u, control | (1u << 2u) | (1u << 6u)) ||

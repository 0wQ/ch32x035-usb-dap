...
_Static_assert(UART_BRIDGE_BAUD_ADJUST_PPM >= -30000 && UART_BRIDGE_BAUD_ADJUST_PPM <= 30000,
               "UART baud adjustment must stay within +/-3 percent");
...
#if UART_BRIDGE_PORT0_ESP_AUTO_RESET || UART_BRIDGE_PORT1_ESP_AUTO_RESET
static void uart_bridge_set_esp_control(GPIO_TypeDef *boot_gpio,
                                        uint32_t boot_pin,
...
{
    if (dtr) {
        GPIO_ResetBits(boot_gpio, boot_pin);
    } else {
        GPIO_SetBits(boot_gpio, boot_pin);
    }
    if (rts) {
...
void uart_bridge_control_line_changed(uint8_t port, bool dtr, bool rts)
{
    if (port == 0u) {
        ...
        uart_bridge_set_esp_control(UART_BRIDGE_PORT0_ESP_BOOT_GPIO,
                                    UART_BRIDGE_PORT0_ESP_BOOT_PIN,
                                    UART_BRIDGE_PORT0_ESP_EN_GPIO,
                                    UART_BRIDGE_PORT0_ESP_EN_PIN,
                                    dtr,
                                    rts);
#else
        (void)dtr;
        ...
        uart_bridge_set_esp_control(UART_BRIDGE_PORT1_ESP_BOOT_GPIO,
                                    UART_BRIDGE_PORT1_ESP_BOOT_PIN,
                                    UART_BRIDGE_PORT1_ESP_EN_GPIO,
                                    UART_BRIDGE_PORT1_ESP_EN_PIN,
                                    dtr,
                                    rts);
#else
        (void)dtr;
...
static void uart_bridge_init_esp_control(void)
{
...
#if UART_BRIDGE_PORT0_ESP_AUTO_RESET
    gpio.GPIO_Pin = UART_BRIDGE_PORT0_ESP_BOOT_PIN;
    GPIO_Init(UART_BRIDGE_PORT0_ESP_BOOT_GPIO, &gpio);
    gpio.GPIO_Pin = UART_BRIDGE_PORT0_ESP_EN_PIN;
    GPIO_Init(UART_BRIDGE_PORT0_ESP_EN_GPIO, &gpio);
    uart_bridge_control_line_changed(0u, false, false);
#endif
#if UART_BRIDGE_PORT1_ESP_AUTO_RESET
    gpio.GPIO_Pin = UART_BRIDGE_PORT1_ESP_BOOT_PIN;
    GPIO_Init(UART_BRIDGE_PORT1_ESP_BOOT_GPIO, &gpio);
    gpio.GPIO_Pin = UART_BRIDGE_PORT1_ESP_EN_PIN;
    GPIO_Init(UART_BRIDGE_PORT1_ESP_EN_GPIO, &gpio);
    uart_bridge_control_line_changed(1u, false, false);
#endif
}
#else
void uart_bridge_control_line_changed(uint8_t port, bool dtr, bool rts)
...
static uint32_t uart_adjust_baud(uint32_t baud_rate)
{
#if UART_BRIDGE_BAUD_ADJUST_PPM == 0
    return baud_rate;
#else
    return (uint32_t)(((int64_t)baud_rate * (1000000 + UART_BRIDGE_BAUD_ADJUST_PPM) + 500000) / 1000000);
#endif
}
...
static void HIGHCODE uart_dma_tx_start(uart_bridge_port_t *port)
{
...
static void uart_apply_line_config(uart_bridge_port_t *port, const usb_serial_line_config_t *config)
{
    ...
    init.USART_BaudRate = uart_adjust_baud(config->baud_rate);
...
#if UART_BRIDGE_PORT0_ESP_AUTO_RESET
                               | UART_BRIDGE_PORT0_ESP_BOOT_GPIO_CLOCK
                               | UART_BRIDGE_PORT0_ESP_EN_GPIO_CLOCK
#endif
#if UART_BRIDGE_PORT1_ESP_AUTO_RESET
                               | UART_BRIDGE_PORT1_ESP_BOOT_GPIO_CLOCK
                               | UART_BRIDGE_PORT1_ESP_EN_GPIO_CLOCK
#endif
                               ,
                           ENABLE);
...
void HIGHCODE uart_bridge_process(void)
{
